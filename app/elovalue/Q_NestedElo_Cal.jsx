...
export default function Q_NestedElo_Cal(props) {
  //S3
  ...
  useEffect(() => {
    // console.log(
    //   "중복선수 데이터, csv->json (H2_Q)결과지 데이터 가져옴..계산고고"
    // );
  });
  ...
  useEffect(() => {
    ...
    //중복 선수 추가(startElo 값 가지고 있음)
    for (var i = 0; i < props.nested.length; i++) {
      arr.push(props.nested[i]);
    }
    //중복 선수 추가된 애들 -> 원위치?암튼 result 맞추려고 위쪽에다 맞춤
    ...
    updateArr.splice(updateArr.length - props.nested.length);
