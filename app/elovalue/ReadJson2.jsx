...
import H1_ReadCsv_EloUpload2 from "./H1_ReadCsv_EloUpload2";
import H2_ReadCsv_EloUpload2 from "./H2_ReadCsv_EloUpload2";

const queryClient = new QueryClient();

export default function ReadJson2() {
  return (
    <QueryClientProvider client={queryClient}>
      {/* R2_Q_ELO.json 파일 읽어오기 */}
      <GetJson />
      <GetJson2 />
    </QueryClientProvider>
  );
...
function GetJson() {
  const { isLoading, error, data } = useQuery({
    ...
    queryFn: () =>
      fetch(
        "https://automanix.s3.ap-northeast-2.amazonaws.com/amx/R2/R2_Q_ELO.json" //R2_Q_ELO 읽기
      ).then((res) => res.json()),
  });
...
function GetJson2() {
  const { isLoading, error, data } = useQuery({
    ...
    queryFn: () =>
      fetch(
        "https://automanix.s3.ap-northeast-2.amazonaws.com/amx/R2/R2_H1_ELO.json" //R2_H1_ELO 읽기
      ).then((res) => res.json()),
  });
  ...
  return (
    <>
      ...
      <H2_ReadCsv_EloUpload2 data={data} />
    </>
  );
}
